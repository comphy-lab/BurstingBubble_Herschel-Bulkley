/**
# V_View - Continuous File Monitoring Basilisk Viewer

This is a modified version of bview that monitors the file for changes
and automatically reloads the visualization when the file is updated.

Typical usage is:

~~~bash
v_view2D /path/to/file/restart
~~~
...
static bool g_debug = true; // Default to verbose for now
...
bool reload_file(const char *path) {
  ...
  // No need to explicitly free old solution, restore() will handle it
